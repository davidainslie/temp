...
class AllYourBaseTest extends FunSuite with Matchers {
  ...
  test("empty list") {
    AllYourBase.rebase(2, List(), 10) should be (Some(List()))
  }

  test("single zero") {
    AllYourBase.rebase(10, List(0), 2) should be (Some(List()))
  }

  test("multiple zeros") {
    AllYourBase.rebase(10, List(0, 0, 0), 2) should be (Some(List()))
  }
