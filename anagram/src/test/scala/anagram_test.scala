...
class AnagramSpecs extends FlatSpec with Matchers {
  ...
  it should "not include identical words" in {
    ...
    val anagrams = detector.matches(Seq("corn", "dark", "Corn", "rank", "CORN", "cron", "park"))
  ...
  it should "detect more anagrams" in {
    ...
    val anagrams = detector.matches(Seq("gallery", "ballerina", "regally", "clergy", "largely", "leading"))
  ...
  it should "treat anagrams as case insensitive" in {
    ...
    val anagrams = detector.matches(Seq("cashregister", "Carthorse", "radishes"))
