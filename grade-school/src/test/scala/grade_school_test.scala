...
class GradeSchoolSpecs extends FunSuite with Matchers with OneInstancePerTest {
  ...
  test ("changing a school's database from the outside (as a client) should not affect said internal database") {
    school.add("Jennifer", 4)
    school.db should be (Map(4 -> Seq("Jennifer")))

    val db = school.db
    db + "Another Jennifer" -> 4
    school.db should be (Map(4 -> Seq("Jennifer")))
  }
}
