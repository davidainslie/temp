...
class TriangleTest extends FlatSpec with Matchers {
  ...
  it should "calc Illogical with 2 zero length side" in {
    Triangle(0, 0, 1).triangleType should equal(TriangleType.Illogical)
  }

  it should "calc Illogical with 3 zero length side" in {
    Triangle(0, 0, 0).triangleType should equal(TriangleType.Illogical)
  }
